...
export function userStatusChange(data: any) {
  return requestClient.putWithMsg(Api.userStatusChange, data);
}
...
export function userRemove(userIds: IDS) {
  return requestClient.deleteWithMsg(`${Api.root}/${userIds}`);
}
...
export function userResetPassword(data: ResetPwdParam) {
  return requestClient.putWithMsg(Api.userResetPassword, data, {
    encrypt: true,
  });
}
